...
import { X, Calendar, Search, Lock } from "lucide-react"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-white w-full max-w-2xl rounded-3xl shadow-2xl flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-200">
                ...
                <div className="p-4 bg-slate-50 border-b border-slate-100 flex flex-wrap gap-4 items-center">
                    ...
                    <div className="flex flex-col gap-1 bg-white px-3 py-2 rounded-xl border border-slate-200 shadow-sm hover:border-atsit-blue/50 transition-colors cursor-pointer min-w-[140px] relative">
                        <span className="text-xs font-bold text-slate-400 uppercase tracking-wider flex items-center gap-1">
                            Desde {plan === 'BASIC' && <Lock className="w-3 h-3 text-violet-400" />}
                        </span>
                        <input
                            type="date"
                            min={plan === 'BASIC' ? toLocalDateString(firstDay) : undefined}
                            value={startDate}
                            onChange={(e) => {
                                if (plan === 'BASIC' && e.target.value < toLocalDateString(firstDay)) {
                                    alert('Visualizar historial de meses pasados es exclusivo del Plan PRO.')
                    ...
                    <div className="ml-auto text-xs font-bold text-slate-400">
                        {loading ? "Cargando..." : `${new Set(transactions.map(t => t.groupId || t.id)).size} registros encontrados`}
                    </div>
                </div>
